...
# Free Cycles
def get_free_plan_cycles(collection_freePlanCycles) -> pd.DataFrame:
# Consultar los documentos, extrayendo solo los campos necesarios
    cursor = collection_freePlanCycles.find({}, {'_id': 0,  # excluye _id
                                    'user_id': 1,
                                    'cycles_consumed': 1,
                                    'ts_last_start': 1,
                                    'max_cycles': 1}
...
def asign_countries_to_free_users(df):
    """
    Asigna el país a cada fila del DataFrame y devuelve el DataFrame actualizado.
...
def convert_timestamp_column(df):
    df = df.copy()
    df['last_date'] = pd.to_datetime(df['ts_last_start'], unit='s').dt.strftime('%Y-%m-%d')
    df = df.drop(columns=['ts_last_start'])
    return df
...
def update_graphs(start_date, end_date):
    ...
    # Free Cycles
    free_cycles_data = get_free_plan_cycles(collection_freePlanCycles)
    free_cycles_data_by_country = asign_countries_to_free_users(free_cycles_data)
    free_cycles_complete = convert_timestamp_column(free_cycles_data_by_country)
    max_consumed_df = free_cycles_complete[free_cycles_complete['cycles_consumed'] == free_cycles_complete['max_cycles']]
    over_max_consumed_df = free_cycles_complete[free_cycles_complete['cycles_consumed'] > free_cycles_complete['max_cycles']]
    
    # Calcular el total de user_count por fecha
    total_by_date = dau_df_by_country.groupby('date')['user_count'].sum().reset_index()
    total_by_month = mau_df_by_country.groupby('month')['user_count'].sum().reset_index()
    ...
    # Agregar la línea envolvente
    dau_fig.add_trace(go.Scatter(x=total_by_date['date'], y=total_by_date['user_count'],
                                 mode='lines+markers', name='Total', line=dict(color='blue', width=2),marker=dict(size=6)))
    ...
    # Agregar la línea envolvente
    mau_fig.add_trace(go.Scatter(x=total_by_month['month'], y=total_by_month['user_count'],
                                 mode = "lines+markers", name="Total", line=dict(color='blue', width=2),marker=dict(size=6)))
